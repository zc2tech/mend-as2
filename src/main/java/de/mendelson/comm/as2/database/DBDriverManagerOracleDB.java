 ...
 * @version $Revision: 3 $
 */
public class DBDriverManagerOracleDB extends AbstractDBDriverManagerMySQL implements ISQLQueryModifier {

    /**
     * keeps this as singleton for the whole server instance
     */
    private static DBDriverManagerOracleDB instance;
    /**
     ...
     */
    public static DBDriverManagerOracleDB instance() {
        if (instance == null) {
            synchronized (DBDriverManagerOracleDB.class) {
                if (instance == null) {
                    instance = new DBDriverManagerOracleDB();
                }
            }
        }
        return (instance);
