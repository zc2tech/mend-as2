 ...
 */
/**
 * MySQL/MariaDB database server wrapper. Like PostgreSQL, MySQL runs as an
...
public class DBServerMySQL implements IDBServer {
    ...
    private final DBDriverManagerMySQL dbDriverManager;
    ...
    /**
     * Initialize MySQL/MariaDB database server wrapper
     */
    public DBServerMySQL(IDBDriverManager driverManager,
        ...
        this.dbDriverManager = (DBDriverManagerMySQL) driverManager;
    }
    ...
    /**
     * Ensure MySQL/MariaDB server is accessible and databases are initialized.
     * Unlike HSQLDB, MySQL runs as an external service.
     */
    @Override
    public void ensureServerIsRunning() throws Exception {
        ...
        SystemEventManagerImplAS2.instance().newEvent(
                ...
                "Connecting to external MySQL server at " + config.getHost() + ":" + config.getPort());

        StringBuilder startupLog = new StringBuilder();
        startupLog.append("MySQL Configuration:\n");
        ...
        try {
            ...
            try (Statement statement = configConnection.createStatement()) {
                // Analyze tables for better query planning (MySQL syntax)
                statement.execute("ANALYZE TABLE version");
            } catch (Exception e) {
                this.logger.warning("Could not run ANALYZE TABLE on CONFIG database: " + e.getMessage());
            }
            ...
            try (Statement statement = runtimeConnection.createStatement()) {
                ...
                this.logger.warning("Could not run ANALYZE TABLE on RUNTIME database: " + e.getMessage());
            }
        ...
        if (foundVersion != -1 && foundVersion < requiredDBVersion) {
            ...
            for (int i = foundVersion; i < requiredDBVersion; i++) {
                ...
                if (!this.startDBUpdate(i, DB_TYPE)) {
                    this.logger.severe(MODULE_NAME + " " + rb.getResourceString("update.error.mysql",
                            new Object[]{String.valueOf(i), String.valueOf(i + 1)}));
                    SystemEvent event = new SystemEvent(SystemEvent.SEVERITY_ERROR,
                            SystemEvent.ORIGIN_SYSTEM,
                            SystemEvent.TYPE_DATABASE_UPDATE);
                    event.setSubject(rb.getResourceString("database." + DB_TYPE));
                    event.setBody(rb.getResourceString("update.error.mysql",
                            new Object[]{String.valueOf(i), String.valueOf(i + 1)}));
                    SystemEventManagerImplAS2.instance().newEvent(event);
                    System.exit(-1);
                }
     ...
     * MySQL server itself continues running as an external service.
     */
    @Override
    public void shutdown() {
        try {
            ...
            SystemEventManagerImplAS2.instance().newEvent(
                    ...
                    "MySQL connection pools closed. External MySQL server continues running.");
    ...
    private boolean startDBUpdate(int startVersion, final int DB_TYPE) {
        ...
        if (DB_TYPE == IDBDriverManager.DB_CONFIG) {
            updateResource = SQLScriptExecutor.SCRIPT_RESOURCE_CONFIG_MYSQL;
        } else if (DB_TYPE == IDBDriverManager.DB_RUNTIME) {
            updateResource = SQLScriptExecutor.SCRIPT_RESOURCE_RUNTIME_MYSQL;
        } else if (DB_TYPE != IDBDriverManager.DB_DEPRICATED) {
