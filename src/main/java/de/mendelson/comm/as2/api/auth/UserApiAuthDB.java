...
package de.mendelson.comm.as2.api.auth;
...
/**
 * Database access layer for user API authentication credentials.
 * Pattern based on UserTrackerAuthDB.
 */
public class UserApiAuthDB {

    /**
     * Load all API auth credentials for a user.
     *
     * @param userId The user's database ID
     * @param configConnection Database connection to config DB
     * @return List of credentials (may be empty)
     */
    public List<UserApiAuthCredential> loadCredentials(int userId, Connection configConnection)
            throws SQLException {
        List<UserApiAuthCredential> credentials = new ArrayList<>();

        String sql = "SELECT id, auth_type, username, password, cert_fingerprint, cert_alias, enabled "
                + "FROM user_api_auth_credentials WHERE user_id=? ORDER BY id";
        ...
        try (PreparedStatement statement = configConnection.prepareStatement(sql)) {
            ...
            try (ResultSet result = statement.executeQuery()) {
                while (result.next()) {
                    UserApiAuthCredential credential = new UserApiAuthCredential();
    ...
    public boolean[] loadMasterToggles(int userId, Connection configConnection)
            throws SQLException {
        String sql = "SELECT api_auth_basic_enabled, api_auth_cert_enabled "
        ...
        try (PreparedStatement statement = configConnection.prepareStatement(sql)) {
            ...
            try (ResultSet result = statement.executeQuery()) {
                if (result.next()) {
                    return new boolean[]{
                        result.getBoolean("api_auth_basic_enabled"),
                        result.getBoolean("api_auth_cert_enabled")
                    };
                }
            }
        }
    ...
    /**
     * Save all API auth credentials for a user. Deletes existing
     ...
     * mode)
     */
    public void saveCredentials(int userId, List<UserApiAuthCredential> credentials,
        ...
        String updateToggles = "UPDATE webui_users SET "
                + "api_auth_basic_enabled=?, api_auth_cert_enabled=? "
                + "WHERE id=?";
        ...
        List<UserApiAuthCredential> validCredentials = new ArrayList<>();
        ...
        if (!validCredentials.isEmpty()) {
            String insertSql = "INSERT INTO user_api_auth_credentials"
            ...
            try (PreparedStatement statement = configConnection.prepareStatement(insertSql)) {
                for (UserApiAuthCredential credential : validCredentials) {
                    statement.setInt(1, userId);
    ...
    /**
     * Delete all API auth credentials for a user.
     *
     * @param userId The user's database ID
    ...
    public void deleteCredentials(int userId, Connection configConnection) throws SQLException {
        String sql = "DELETE FROM user_api_auth_credentials WHERE user_id=?";
        try (PreparedStatement statement = configConnection.prepareStatement(sql)) {
            statement.setInt(1, userId);
            statement.executeUpdate();
        }
