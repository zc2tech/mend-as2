...
# - target/mend-as2-1.1.3-full.jar (~120 MB)
# - target/mend-as2-1.1.3-full-full.tar.gz (~2.6 MB)
# - target/mend-as2-1.1.3-full-full.zip (~2.6 MB)
```

**Verify full distribution:**
```bash
# Extract and check contents
tar -tzf target/mend-as2-1.1.3-full-full.tar.gz | head -20

# Should include:
# - mend-as2-1.1.3-full.jar
...
# Expected output files:
# - target/mend-as2-1.1.3-headless.jar (~100-105 MB)
# - target/mend-as2-1.1.3-headless-headless.tar.gz (~2.6 MB)
# - target/mend-as2-1.1.3-headless-headless.zip (~2.6 MB)
```

**Verify headless distribution:**
```bash
# Extract and check contents
tar -tzf target/mend-as2-1.1.3-headless-headless.tar.gz | head -20

# Should include:
# - mend-as2-1.1.3-headless.jar
...
Expected files:
```
mend-as2-1.1.3-full-full.tar.gz         (~2.6 MB)
mend-as2-1.1.3-full-full.zip            (~2.6 MB)
mend-as2-1.1.3-headless-headless.tar.gz (~2.6 MB)
mend-as2-1.1.3-headless-headless.zip    (~2.6 MB)
```
...
<version>1.1.3</version>  <!-- UPDATE THIS -->
...
```bash
# Extract
tar -xzf target/mend-as2-1.1.3-full-full.tar.gz
cd mend-as2-1.1.3-full
...
```bash
# Extract
tar -xzf target/mend-as2-1.1.3-headless-headless.tar.gz
cd mend-as2-1.1.3-headless
...
3. Choose tag: `v1.1.3`
4. Release title: `Mend AS2 v1.1.3`
5. Description: Copy from CHANGELOG.md
6. Upload artifacts:
   - `mend-as2-1.1.3-full-full.tar.gz`
   - `mend-as2-1.1.3-full-full.zip`
   - `mend-as2-1.1.3-headless-headless.tar.gz`
   - `mend-as2-1.1.3-headless-headless.zip`
...
gh release create v1.1.3 \
  target/mend-as2-1.1.3-full-full.tar.gz \
  target/mend-as2-1.1.3-full-full.zip \
  target/mend-as2-1.1.3-headless-headless.tar.gz \
  target/mend-as2-1.1.3-headless-headless.zip \
  --title "Mend AS2 v1.1.3" \
...
**Download:**
- [mend-as2-1.1.3-full-full.tar.gz](URL) (Linux/macOS)
- [mend-as2-1.1.3-full-full.zip](URL) (Windows)
...
**Download:**
- [mend-as2-1.1.3-headless-headless.tar.gz](URL) (Linux/macOS)
- [mend-as2-1.1.3-headless-headless.zip](URL) (Windows)
...
tar -xzf mend-as2-1.1.3-headless.tar.gz
cd mend-as2-1.1.3-headless
...
**Full Changelog**: https://github.com/zc2tech/mend-as2/compare/v1.0.0...v1.1.3
```
...
wget https://github.com/zc2tech/mend-as2/releases/download/v1.1.3/mend-as2-1.1.3-full-full.tar.gz
tar -tzf mend-as2-1.1.3-full-full.tar.gz | head -20

wget https://github.com/zc2tech/mend-as2/releases/download/v1.1.3/mend-as2-1.1.3-headless-headless.tar.gz
tar -tzf mend-as2-1.1.3-headless-headless.tar.gz | head -20
```
...
nano pom.xml  # Change <version>1.1.3</version> to <version>1.2.0-SNAPSHOT</version>
git add pom.xml
